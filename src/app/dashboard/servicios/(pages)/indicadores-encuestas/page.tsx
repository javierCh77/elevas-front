...
export default function EncuestaDashboardPage() {
  ...
  const handleDescargarPDF = async () => {
    ...
    pdf.save(`reporte-${empresa}.pdf`);
  };
  

  const enpsPromedioNum = respuestas.length ? respuestas.reduce((sum, r) => sum + (r.respuestas.recomendariaEmpresa || 0), 0) / respuestas.length : 0;
  const enpsPromedio = enpsPromedioNum.toFixed(1);
  const reconocimientoRaw = respuestas.filter((r) => r.respuestas.trabajoValorado >= 4).length;
  const reconocimiento = respuestas.length ? ((reconocimientoRaw / respuestas.length) * 100).toFixed(0) + "%" : "0%";
  const equilibrioRaw = respuestas.filter((r) => r.respuestas.equilibrioVidaLaboral >= 4).length;
  const equilibrio = respuestas.length ? ((equilibrioRaw / respuestas.length) * 100).toFixed(0) + "%" : "0%";
  const equilibrioNum = respuestas.length ? (equilibrioRaw / respuestas.length) * 100 : 0;
  const riesgoRotacion = enpsPromedioNum < 3 && equilibrioNum < 50 ? "Alto" : "Bajo";
  const riesgoDescripcion = riesgoRotacion === "Alto" ? "Baja recomendación y satisfacción" : "Estabilidad positiva percibida";
  ...
  return (
    <div className="p-4 space-y-6 h-[70vh] overflow-y-auto max-w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:gap-4 gap-2">
        <input type="text" placeholder="Nombre de la empresa" className="p-2 border border-[#DEDFA9] rounded-md bg-[#F8F8EE] text-sm w-full md:w-1/3" value={empresa} onChange={(e) => setEmpresa(e.target.value)} />
        <div className="flex gap-2 w-full md:w-1/3">
          <input type="date" className="p-2 border border-[#DEDFA9] rounded-md bg-[#F8F8EE] text-sm w-1/2" value={fechaDesde} onChange={(e) => setFechaDesde(e.target.value)} />
          <input type="date" className="p-2 border border-[#DEDFA9] rounded-md bg-[#F8F8EE] text-sm w-1/2" value={fechaHasta} onChange={(e) => setFechaHasta(e.target.value)} />
        </div>
        <input type="number" placeholder="Cantidad de personas" className="p-2 border border-[#DEDFA9] rounded-md bg-[#F8F8EE] text-sm w-full md:w-1/3" value={cantidad} onChange={(e) => setCantidad(e.target.value)} />
        <button onClick={handleEnviar} className="px-4 py-2 text-sm bg-[#AEA344] text-white rounded-md shadow-md hover:bg-[#8C8631]">Aplicar</button>
      </div>
      ...
      <div ref={dashboardRef}>
        {mostrarInfo && (
          <div className="bg-[#F8F8EE] border border-[#DEDFA9] rounded-xl p-4 shadow-sm">
            ...
            <p className="text-sm text-[#4B4B3D]">Cantidad de personas: <strong>{respuestas.length}/{cantidad}</strong></p>
          </div>
        )}
