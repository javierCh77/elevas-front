...
export default function EncuestaDashboardPage() {
  ...
  useEffect(() => {
    const obtenerAreas = async () => {
      try {
        ...
        const todasAreas = data.map((r) => r.areaTrabajo).filter(Boolean);
        const unicas = Array.from(new Set(todasAreas)).sort();
  ...
  const parseRespuestas = (respuestas: RespuestaEncuesta[]): RespuestaEncuesta[] => {
    return respuestas.map((r) => {
      ...
      return { ...r, respuestas: respuestasNormalizadas, };
    });
  };
      ...
      const filtradas = data.filter((item: RespuestaEncuesta) => {
        ...
        return (
          ...
          (!areaTrabajo ||
            item.areaTrabajo
              .toLowerCase()
              .includes(areaTrabajo.toLowerCase())) &&
  ...
  const enpsPromedioNum = respuestas.length
    ? respuestas.reduce((sum, r) => sum + (r.respuestas.recomendariaEmpresa ?? 0), 0 ) / respuestas.length: 0;

  const enpsPromedio = enpsPromedioNum.toFixed(1);
  const reconocimientoRaw = respuestas.filter((r) => (r.respuestas.reconocimientoValorado ?? 0) >= 4).length;
  const reconocimiento = respuestas.length ? ((reconocimientoRaw / respuestas.length) * 100).toFixed(0) + "%" : "0%";
  const equilibrioRaw = respuestas.filter((r) => (r.respuestas.climaEquilibrioVida ?? 0) >= 4 ).length;
  ...
  return (
    <div className="p-4 space-y-6 h-[92vh] overflow-y-auto max-w-full">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-2xl font-bold text-[#322616]"> Indicadores de encuesta Experiencia Colaborador </h2>
      </div>

      {/* Filtros */}
      <div className="flex flex-col md:flex-row md:items-center md:gap-4 gap-2">
        <div className="flex flex-col w-full md:w-1/3">
          <label className="text-sm text-[#4B4B3D] mb-1">  Nombre de la empresa </label>
          <input
            type="text"
        ...
        <div className="flex flex-col w-full md:w-1/3">
          <label className="text-sm text-[#4B4B3D] mb-1">Área de trabajo</label>
          <select
            className="p-2 border border-[#DEDFA9] rounded-md bg-[#F8F8EE] text-sm"
            value={areaTrabajo}
        ...
        <div className="flex flex-col w-full md:w-1/3">
          <label className="text-sm text-[#4B4B3D] mb-1"> Cantidad de personas (opcional) </label>
          <input
            type="number"
            placeholder="Ej. 150"
            className="p-2 border border-[#DEDFA9] rounded-md bg-[#F8F8EE] text-sm"
      ...
      <div className="flex justify-end">
        ...
        >
          <Printer size={18} /> Exportar como PDF</button>
      </div>
      {/* Reporte */}
      {mostrarInfo && (
        <div className="animate-fade-in transition-opacity duration-500 opacity-100 space-y-6"  ref={printRef} >
          <div className="bg-[#F8F8EE] border border-[#DEDFA9] rounded-xl p-4 shadow-sm">
            ...
            <p className="text-sm text-[#4B4B3D]">  Cantidad de personas encuestadas:{" "} <strong> {respuestas.length} {cantidad ? ` / ${cantidad}` : ""} </strong> </p>
          </div>
          <LeyendaIndicadores />
          ...
          <DiagnosticoResumen  compromiso={compromiso} satisfaccion={satisfaccion} enps={enpsPromedioNum} riesgo={riesgoRotacion} />
        </div>
      )}
    </div>
  );
